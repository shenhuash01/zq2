    ...
    getzq_cookie2()
     $.done()
         ...
         for (let k = 0; k < zq_cookie2Arr.length; k++) {
             ...
             zq_cookie21 = cookie_id + '&' + bodyVal
             zq_cookie22 = 'uid='+zq_cookie2Arr[k].split('&uid=')[1] + '&'+ bodyVal
             //待处理cookie
             console.log(`${zq_cookie21}`)
             console.log(`--------第 ${k + 1} 个账号收益查询中--------\n`)
             await nickname(zq_cookie22)
             if ($.message.length != 0) {
                 message += "账号" + (k + 1) + "：  \n" + $.message + " \n"
             }
             await $.wait(4000);
...
function nickname(zq_cookie22,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.wkandian.com/v17/NewTask/getSign.json?'+ zq_cookie22,
            headers : {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if(result.success == true){
                    ...
                    await $.wait(1000);
                    await today_score(zq_cookie21,nickname1)
...
function today_score(zq_cookie21,nickname1,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.wkandian.com/wap/user/balance?'+ zq_cookie21,
            headers : {
    'Host': 'kandian.wkandian.com'
},
            }
