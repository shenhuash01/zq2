/*
中青自动提现 zq_withdraw2.js，
调用zq_cookie2,zq_withdraw2,zq_cash
...
脚本地址：
https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/zq/zq_withdraw2.js
定时确保在每日首次火爆转发之后（zq_share.js），一天一次
***************************************************************************
v2p食用说明：
1.添加重写如下：
https://kandian.wkandian.com/v5/wechat/withdraw2.json 重写目标 https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/zq/zq_withdraw2.js
2.打开app进入提现页面，选择对应金额，点击立即提现（不管当日是否已经提现，都能抓），到变量中查看到有zq_withdraw2即可
3.注意多账号用户抓包zq_withdraw2时需要与zq_cookie2账号顺序一致
4.zq_cash不填默认0.3元，填的话有0.3，30两个选项（boxjs订阅中可直接修改，没有订阅的话直接新建变量即可）
5.zq_withdraw2务必与zq_cash金额一致，即修改zq_withdraw2时也要修改zq_cash
****************************************************************************
青龙食用说明：
1.自己抓包数据
2.export zq_withdraw2="p=xxx"(多账号用@隔开)
3.export zq_cash="0.3"或者export zq_cash="30"，不填默认30
4.多账号用户抓包zq_withdraw2时需要与zq_cookie2账号顺序一致
5.zq_withdraw2务必与zq_cash金额一致，即修改zq_withdraw2时也要修改zq_cash
*/
...
message = ""
let zq_withdraw2= $.isNode() ? (process.env.zq_withdraw2 ? process.env.zq_withdraw2 : "") : ($.getdata('zq_withdraw2') ? $.getdata('zq_withdraw2') : "")
let zq_withdraw2Arr = []
let zq_withdraw2s = ""
...
if (zq_withdraw2) {
    if (zq_withdraw2.indexOf("@") == -1 && zq_withdraw2.indexOf("@") == -1) {
        zq_withdraw2Arr.push(zq_withdraw2)
    } else if (zq_withdraw2.indexOf("@") > -1) {
        zq_withdraw2s = zq_withdraw2.split("@")
    } else if (process.env.zq_withdraw2 && process.env.zq_withdraw2.indexOf('@') > -1) {
        zq_withdraw2Arr = process.env.zq_withdraw2.split('@');
        console.log(`您选择的是用"@"隔开\n`)
    }
} else if($.isNode()){
    var fs = require("fs");
    zq_withdraw2 = fs.readFileSync("zq_withdraw2.txt", "utf8");
    if (zq_withdraw2 !== `undefined`) {
        zq_withdraw2s = zq_withdraw2.split("\n");
    } else {
...
Object.keys(zq_withdraw2s).forEach((item) => {
    if (zq_withdraw2s[item] && !zq_withdraw2s[item].startsWith("#")) {
        zq_withdraw2Arr.push(zq_withdraw2s[item])
    }
})
    ...
    getbody()
     $.done()
         ...
         for (let k = 0; k < zq_cookie2Arr.length; k++) {
             ...
             zq_cookie22 = cookie_id + '&' + bodyVal
             //待处理cookie
             console.log(`--------第 ${k + 1} 个账号收益查询中--------\n`)
             zq_withdraw21 = zq_withdraw2Arr[k]
             await today_score(zq_cookie22)
...
function withdraw(zq_withdraw21,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            ...
            'Content-Length': zq_withdraw21.length.toString(),
...
function getbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/wechat\/withdraw2.json/)) {
          bodyVal=$request.body
            console.log(bodyVal)
        if (zq_withdraw2) {
            if (zq_withdraw2.indexOf(bodyVal) > -1) {
                $.log("此提现请求已存在，本次跳过")
            } else if (zq_withdraw2.indexOf(bodyVal) == -1) {
                zq_withdraw2s = zq_withdraw2 + "@" + bodyVal;
                $.setdata(zq_withdraw2s,'zq_withdraw2');
                $.log(`${$.name}获取提现: 成功, zq_withdraw2s: ${bodyVal}`);
                bodys = zq_withdraw2s.split("@")
                 $.msg($.name, "获取第" + bodys.length + "个提现请求: 成功🎉", ``)
            }
        } else {
            $.setdata($request.body,'zq_withdraw2');
            $.log(`${$.name}获取提现: 成功, zq_withdraw2s: ${bodyVal}`);
...
function today_score(zq_cookie22,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.wkandian.com/wap/user/balance?'+ zq_cookie22,
            headers : {
    'Host': 'kandian.wkandian.com'
},
            }
