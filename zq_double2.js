    ...
    getzq_cookie2()
     $.done()
         ...
         for (let k = 0; k < zq_cookie2Arr.length; k++) {
             ...
             $.message = ""
             bodyVal = zq_cookie2Arr[k];
             //cookie = bodyVal.replace(/zqkey=/, "cookie=")
             //cookie_id = cookie.replace(/zqkey_id=/, "cookie_id=")
             zq_cookie21 = '&device_brand=xfdg&device_id=cc7dgdsgfsz83e&device_model=1gx&device_platform=android&device_type=android&inner_version=202107261526&mi=0&openudid=cc7dgdsgfsz83e&os_api=27&os_version=bdftgsdfga&phone_network=WIFI&phone_sim=1' + '&request_time=' + time1 + '&' + bodyVal
             //待处理cookie
             console.log(`${zq_cookie21}`)
             console.log(`--------第 ${k + 1} 个账号阅读翻倍中--------\n`)
             await today_score(zq_cookie21)
             if ($.message.length != 0) {
...
function today_score(zq_cookie21,timeout = 0) {
    return new Promise((resolve) => {
        let url = {
            url : 'https://kandian.wkandian.com/v17/NewTask/setJoinTaskPromotion.json?'+ zq_cookie21,
            headers : {
